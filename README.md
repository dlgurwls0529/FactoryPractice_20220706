...
왜냐하면, 이 디자인 상의 수정 작업에서 기존 클래스를 안 건드리므로,  
이에 따라 발생할 수 있는 코드 변화량이 줄어들기 때문이다.  

가령 팩토리 메소드 패턴이 적용된 것과 정적 팩토리 클래스에 대해서,
Client 클래스가 Product 를 생성한다고 하자.  
Client가 기존에 없던 Product를 요구한다면  
(Client 코드가 새로운 Product를 요구하는 방식으로 변화)  

팩토리 메소드 패턴이 적용된 디자인에서는 새로운  
ConcreteFactory를 추가하여 Client 코드의 해당 부분을 수정한다.  
즉, Client 코드에서 1줄이 수정되었다.  

정적 팩토리 클래스에서는 팩토리 메소드에서  
분기문에 조건 하나를 추가하고 새로운 Product를 생성하는 부분을 넣는다.  
생성에 필요한 작업을 뭉뚱그려서 대략 3줄이라 하자.  
그리고 Client 코드에서 해당 부분을 수정한다.(createShip("blackShip"->"whiteShip"))  
즉, Client 코드에서 1줄, 팩토리 클래스에서 3줄이 수정되었다.

구조가 복잡해지면, 특정 클래스의 변화(예시에서는 Client 클래스의 변화)  
에 따라서, OCP를 만족하지 않는 디자인이라면 다른 클래스가 더 크게 변화해야 할 것이다.(강한 결합의 정의)  
이 예시에서는 정적 팩토리 메소드 예시에서 같은 클래스 변화에 4줄이 변경되었지만,  
실전에서는 더 큰 결합도의 차이를 보일 것이라는 말이다.  
...
(WhiteShipFactory 나 BlackShipFactory 없애면 Container 하나에만 빨간줄 뜨니까)  
즉, 클래스 간의 결합도가 낮아진다. https://kotlinworld.com/64
...
### 4.3. JAVA에서 다형성의 구현 방법
- 첫째, 슈퍼 클래스(인터페이스)와 서브 클래스  
- 둘째, 메소드 오버라이딩  
- 셋째, 상위 타입 객체에 하위 타입 객체 대입 (업캐스팅)  

이후 상위 타입 형식으로 자식 메소드를 호출하여 활용한다.  
